...
test('all form controls use the shared interactive hover and active border', () => {
  const tokens = text('src/styles/tokens.css');
  const styles = text('src/styles/system.css');
  const account = text('src/pages/account-pages.tsx');
  assert(tokens.includes('--ds-control-border-hover: #3d3d3d'));
  assert(styles.includes('.ds-input:not(:disabled):hover'));
  assert(styles.includes('.ds-select-trigger[data-state=\'open\']'));
  assert(styles.includes('.ds-search:hover, .ds-search:focus-within'));
  assert(styles.includes('.ds-account-input-group:hover, .ds-account-input-group:focus-within'));
  assert(styles.includes('.ds-account-team-chip:hover, .ds-account-team-chip:focus-within'));
  assert(!account.includes('ds-account-hover-field'));
  assert(account.includes('aria-label="Remove default team"'));
});
