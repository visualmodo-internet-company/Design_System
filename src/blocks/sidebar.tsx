...
import { ArrowLeft, Bell, ChevronRight, MoreHorizontal, Search } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Avatar, AvatarFallback } from '@/components/ui/avatar';
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuLabel, DropdownMenuItem } from '@/components/ui/dropdown-menu';
import { TeamSwitcher } from './team-switcher';
import type { NavigationGroup, NavigationItem } from './navigation';
export interface SidebarProps { groups: NavigationGroup[]; activeId: string; onNavigate: (id: string) => void; workspace: string; workspacePlan?: string; userName: string; onSearch: () => void; onAction: (action: string) => void; onClose?: () => void }
export function Sidebar({ groups, activeId, onNavigate, workspace, workspacePlan, userName, onSearch, onAction, onClose }: SidebarProps) {
  ...
  return <div className="ds-sidebar-inner">
    <div className="ds-sidebar-head">
      ...
      />
      <button className="ds-find" type="button" onClick={onSearch}><Search aria-hidden="true" /><span>Find</span><kbd className="ds-kbd">F</kbd></button>
    </div>
