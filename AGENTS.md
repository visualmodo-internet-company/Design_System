...
- Input interaction is global: text inputs, textareas, selects, search wrappers and grouped Account controls use the shared `--ds-control-border-hover` border on hover, focus and open/active states. In dark mode the measured reference value is `#3d3d3d`. Default Team remove uses a 16 px Lucide X with 1.5 stroke and no hover fill.
