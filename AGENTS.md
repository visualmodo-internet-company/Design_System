...
- The sidebar user menu is a dedicated pattern in `src/blocks/user-menu.tsx`: 330 px desktop width, profile/settings row first, Feedback/Home Page/Changelog/Help/Docs/Log Out rows with right-aligned icons, a 36 px Upgrade to Pro action, and the All systems normal status row at the bottom. Do not replace it with a generic two-item dropdown.
- The header project selector is a dedicated pattern in `src/blocks/project-switcher.tsx`: 40 px trigger, 440 px desktop panel, 52 px Find Project search header, 40 px project row, separator and 40 px Create Project row. Project and Create rows are transparent at rest and use the shared hover fill. Do not replace it with a generic Dropdown Menu.
...
- Account field hover is not global: only the Display Name text input uses the stronger hover border in the supplied Settings reference. Username group, Default Team chip, billing fields and other Account controls keep their base border on hover. Default Team remove uses a 16 px Lucide X with 1.5 stroke and no hover fill.
